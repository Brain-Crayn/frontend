...
const DynamicForm = () => {
  ...
  useEffect(() => {
    // Fetch CSRF token logic here if needed
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/contributeRequest/requestform/`, {
        method: "POST",
        headers: {
...
const DynamicForm = () => {
  ...
  const [submitMessage, setSubmitMessage] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [canSubmit, setCanSubmit] = useState(true); // Rate limiter state

  useEffect(() => {
    // Fetch CSRF token logic if needed
  }, []);
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      if (file.type === "application/pdf" || file.type.startsWith("image/")) {
        ...
        window.alert("Please upload a valid image or PDF file.");
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!canSubmit) {
      window.alert("You can only submit once every 45 seconds. Please wait before submitting again.");
      return;
    }
    ...
    try {
      ...
      setSubmitMessage("Your form has been successfully submitted (you get one submission ever 45 seconds)!");
      ...
      // Enable submit button again after 45 seconds
      setTimeout(() => {
        setCanSubmit(true);
      }, 45000);
      ...
      console.error("FAILED:", error);
      setSubmitMessage("There was an error submitting your form. Please try again.");
      setCanSubmit(true); // Allow retry on failure
    } finally {
      setIsLoading(false);
    }
  };
